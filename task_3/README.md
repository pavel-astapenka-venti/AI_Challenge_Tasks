...
1. **Create a Telegram bot** via BotFather:
   ...
   - Set commands with `/setcommands`:
     ...
     learn - Submit a URL to learn from (usage: /learn URL)
...
2. **Import the workflow** into n8n:
   ...
   - Select the `Learning Assistant Bot.json` file from this repository.

3. **Configure Telegram credentials**:
   - In n8n, go to **Credentials** (/home/credentials).
   - Add a new **Telegram API** credential.
   - Paste your bot token and save.
   - Update all Telegram nodes in the workflow to use this credential.

4. **Activate the workflow**:
   - Open the imported workflow.
   - Toggle the **Active** switch to enable it.
   - The Telegram webhook will be registered automatically.

5. **Test the bot**:
   - Open your bot in Telegram and send `/start`.
   - Try `/learn https://en.wikipedia.org/wiki/Photosynthesis` to verify material processing.
   - Send `/quiz` to test quiz generation.
